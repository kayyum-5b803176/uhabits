...
class NumberDialog : AppCompatDialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        view.minusBtn.setOnClickListener {
            val current = view.value.text.toString().toDoubleOrNull() ?: 0.0
            val next = maxOf(0.0, current - 1.0)
            view.value.setText(DecimalFormat("#.##").format(next))
        }
        view.plusBtn.setOnClickListener {
            val current = view.value.text.toString().toDoubleOrNull() ?: 0.0
            view.value.setText(DecimalFormat("#.##").format(current + 1.0))
        }
